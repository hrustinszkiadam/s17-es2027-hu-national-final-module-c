...
A fully functional **main backend** is provided for this module at `http://localhost:5000`. You can find the reference of all endpoints of the main backend in OpenAPI format in [/assets/api/ssa-main-backend-openapi.yaml](/assets/api/ssa-main-backend-openapi.yaml).
...
Running `docker compose up -d` from the `assets/` directory starts two services:

- **`main-backend`**: the provided SkillShare Academy main backend, exposed at `http://localhost:5000`.
- **`db`**: a MySQL server. On first startup it automatically imports both database dumps from `assets/db/` — `ssa_main_backend.sql` into the main backend's own database, and `ssa_content_service.sql` into the database your content service must connect to.
- **`phpmyadmin`**: a web-based MySQL administration tool. It is exposed at `http://localhost:8080` and can be used to manage the database.

The content service you implement is **not** part of this Compose file; you build and run it yourself, connecting it to the seeded `ssa_content_service` database at `localhost:3306` (user `root`, password `toor`).
...
**Long-lived test token:** For testing without repeated login, you can use this pre-issued Bearer token for **alice@example.com** (payload subject `1`). The API Info endpoint of the main backend `/api/v1` provides this token upon visitation. If the token were to expire or get revoked, visiting the endpoint shall generate a new test token.
...
**Reference:** [/assets/handouts/handout-hmac-sha256-token-signing.md](/assets/handouts/handout-hmac-sha256-token-signing.md) (signing and verifying `header.payload.signature` with HMAC-SHA256).
...
**API Documentation:** OpenAPI specification for the content service is available in the `assets` directory: [/assets/api/ssa-content-service-openapi.yaml](/assets/api/ssa-content-service-openapi.yaml).

**Database:** A database dump for the content service will be provided; its structure is described in the [Database Structure](#database-structure) section above. Competitors must **not** modify the schema. The database will be restored to the original state before marking. A dump for the main backend (users, enrollments, etc.) is also available in `assets/db`; it is loaded automatically by the local environment described in [Setup](#setup) and does not apply to the content service.
